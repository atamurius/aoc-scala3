...
object coord:
  ...
  trait Vec[C]:
    type Item
    def components(v: C): Iterator[Item]
    def build(xs: IterableOnce[Item]): C
    val size: Int
    
    def zip(a: C, b: C)(f: (Item, Item) => Item): C = build(for (a,b) <- components(a) zip components(b) yield f(a,b))

    extension(a: C):
      def components: Iterator[Item] = components(a)
  ...
  given Vec[Int2] with {
    type Item = Int
    def components(v: Int2): Iterator[Int] = Iterator(v.x, v.y)
  ...
  given Vec[Int3] with {
    type Item = Int
    def components(v: Int3): Iterator[Int] = Iterator(v.x, v.y, v.z)
  ...
  given Vec[Int4] with {
    type Item = Int
    def components(v: Int4): Iterator[Int] = Iterator(v.x, v.y, v.z, v.w)
